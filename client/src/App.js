...
const App = () => {
    ...
    const loadProjects = () => {
        API.getPortfolioProj('active=true').then(res => {
    ...
    return (
        <ReCaptchaProvider siteKeyV2={RECAPTCHA_V2_SITE_KEY}>
            <Router>
                <Switch>
                    <Route exact path="/" render={props => <Home {...props} projects={projects.filter(project => project.starred)} />} />
